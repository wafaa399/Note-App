...
function showDetailScreen() {
  ...
  if (isMobile()) {
    ...
    // على الشاشات الكبيرة (Desktop) بتظل الشاشتين معاً أو حسب تصميمك
    if (detailSection) detailSection.classList.remove("hidden");
  }
}
function refreshList() {
  ...
  renderNotesList({
    ...
    onSelect: (id) => {
      ...
      showDetailScreen(); // لما تدوسي على النوت هيظهر فقط التفاصيل وتختفي القائمة على الموبايل
    },
    onDelete: (id) => {
      Model.deleteNote(id);
      refreshList();
  ...
  document.querySelectorAll(".nav-item").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (view === "add") {
        ...
        // عند الضغط على Notes ترجع القائمة وتختفي التفاصيل
        renderNoteDetail(null);
        showListScreen();
      }
      closeMobileSidebar();
