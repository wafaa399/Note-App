import { formatDate, truncate, clear } from "./helpers.js";

function noteItemHTML(note) {
  return `
    <div class="note-item cursor-pointer px-2 py-2 rounded hover:bg-gray-50" data-id="${note.id}">
      <p class="font-medium text-[#303030] text-[0.95rem]">${note.title}</p>
      <p class="text-gray-400 text-xs mt-1">${truncate(note.content, 70)}</p>
