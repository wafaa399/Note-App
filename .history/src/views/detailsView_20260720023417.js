...
export function renderAddNoteForm({ onSubmit }) {
  const container = document.getElementById("detailView");
  container.innerHTML = `
    <h2 class="text-2xl font-semibold text-[#303030] mb-6">Add Note</h2>
    <div class=" w-[37.5rem] h-[34.625rem] space-y-4 max-w-md">
      <div>
        <label class="block text-gray-500 mb-1">Title*</label>
        <input id="noteTitle" type="text" class="w-full border border-gray-200 rounded px-3 py-2 outline-none focus:ring-1 focus:ring-red-400" />
      </div>
      <div>
        <label class="block text-gray-500 mb-1">Author*</label>
        <input id="noteAuthor" type="text" class="w-full border border-gray-200 rounded px-3 py-2 outline-none focus:ring-1 focus:ring-red-400" />
      </div>
      <div>
        <label class="block text-gray-500 mb-1">Your Note*</label>
        <textarea id="noteContent" rows="6" class="w-full border border-gray-200 rounded px-3 py-2 outline-none focus:ring-1 focus:ring-red-400"></textarea>
      </div>
      <div class="flex gap-3">
        <button id="addNoteBtn" class="bg-[#EC7160] text-white px-4 py-2 rounded hover:bg-[#e05f4d]">Add Note</button>
        <button id="addPinnedBtn" class="bg-gray-100 text-[#EC7160] px-4 py-2 rounded hover:bg-gray-200">Add pinned note</button>
      </div>
    </div>
  `;
