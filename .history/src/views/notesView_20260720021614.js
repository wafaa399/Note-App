...
function noteItemHTML(note) {
  return `
    <div class="w-[16.625rem] h-[6.625rem] note-item cursor-pointer px-2 py-2 rounded hover:bg-gray-50" data-id="${note.id}">
      <p class="font-medium text-[#303030] leading-6 text-[1rem]">${note.title}</p>
      <p class="w-[16.5625rem] h-[2.5rem] text-[#898989]  text-[.82rem] font-medium leading-5  mt-1">${truncate(note.content, 70)}</p>
      <div class="flex justify-between items-center mt-1">
        <span class="text-[#898989] text-[.82rem]  leading-5  font-medium mt-2">${formatDate(note.date)}</span>
        <button class="delete-btn text-[#D82700] text-sm pb-[3px]" data-id="${note.id}">Delete</button>
      </div>
    </div>
  `;
}
